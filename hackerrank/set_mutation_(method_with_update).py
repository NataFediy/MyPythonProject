# Using .update(..) or <method>_update(..) operations creates mutations
# to a set
#
# TASK:
# You are given a set A and N number of other sets.
# These N number of sets have to perform some specific mutation operations
# on set A.
...
# The second line of each part contains space separated list of elements
# in the other set.
#
# Output Format:
...
for _ in range(int(input())):
    method = input().split()
    members_list = set(map(int, input().split()))
    exec('A.{0}({1})'.format(method[0], members_list))
print(sum(A))
